...
export default function NavBar({title, icons}: PropType) {
  ...
  return (
    <nav className='bg-white px-3 flex flex-row h-[50px] justify-between items-center z-10 max-w-[575px] w-full '>
      ...
      <Logo image={ `https://${projectName}.tsdsolution.net/assets/uploads/logos/${metadata?.icons}` }/>
        <div>
          <p className='font-niradei font-bold w-30 text-black-600 font-akbalthom-moul-4 text-xl max-[600px]:text-sm max-[450px]:text-[14px]'>{metadata?.title}</p>
        </div>
      </div>
