  ...
  export default function FilterBar({ isOpen, onToggle, onFilterChange,resetTrigger }: FilterBarProps) {
    const { t } = useLanguage();
  ...
  const newPrices = selectedPrices.includes(label)
    ? selectedPrices.filter(price => price !== label)
    : [...selectedPrices, label];
...
const handlePiecesChange = (label: string) => {
  const newPieces = selectedPieces.includes(label)
    ? selectedPieces.filter(piece => piece !== label)
    : [...selectedPieces, label];
...
const handleAgeChange = (label: string) => {
  const newAges = selectedAges.includes(label)
    ? selectedAges.filter(age => age !== label)
    : [...selectedAges, label];
...
useEffect(() => {
 ...
 return (
  <div className="mt-4 w-[80px] me-2">
    {/* Toggle Button */}
    <div className="relative">
     <button
          onClick={onToggle}
          className={`w-full items-center px-4 py-1 max-[370px]:text-[13px] max-[320px]:text-[10px] text-[13px] rounded-full border-2 focus:outline-none font-niradei ${
    ...
    {/* Collapsible Panel */}
    <div className="relative w-[360px] sm:w-[400px] md:w-[530px] lg:w-[527px]"
     style={{
     width: window.innerWidth <= 320 ? '290px' : window.innerWidth <= 360 ? '330px'  : undefined,}}>
      ...
      >
        <div className="bg-white border rounded-lg p-4 mt-2 shadow relative">
          {/* Close Icon */}
          <span onClick={onToggle} className="absolute bottom-[-15px] right-[50%] translate-x-[50%] bg-black font-niradei text-[12px] p-2 rounded-full shadow-sm z-10 cursor-pointer">
          ...
          {/* Price Filter */}
          <div className="mb-4">
            <label className="text-md font-bold mb-1 font-niradei flex">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512" className='w-4 h-4 mt-1'>
            ...
            {["$0 - $49.99", "$50 - $99.99", "$100+"].map((label, index) => (
              <div className="form-check flex items-center" key={index}>
                <input className="form-check-input w-[15px] h-[15px]" type="checkbox" checked={selectedPrices.includes(label)} onChange={() => handlePriceChange(label)} id={`price-${index}`} />
                <label className="form-check-label ms-2 font-niradei text-[14px] pt-1" htmlFor={`price-${index}`}>{label}</label>
              </div>
            ))}
          </div>
          ...
          <div className="mb-4">
            <label className="text-md font-bold mb-1 font-niradei flex">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 512" className='w-6 h-6 me-1'>
            ...
            {["0 - 5", "6 - 9", "10 - 17", "18+"].map((label, index) => (
              <div className="form-check flex items-center" key={index}>
                <input className="form-check-input w-[15px] h-[15px]" type="checkbox" checked={selectedAges.includes(label)} onChange={()=> handleAgeChange(label)} id={`age-${index}`} />
                <label className="form-check-label ms-2 font-niradei text-[14px] pt-1" htmlFor={`age-${index}`}>{label}</label>
              </div>
            ))}
          </div>
           <button
              onClick={() => {
                ...
                setSelectedPieces([]);
                setSelectedAges([]);
                onFilterChange({ price: [], pieces: [], age: [] }); 
              }}
              className="w-full items-center px-4 py-1 mb-2 max-[370px]:text-[13px] max-[320px]:text-[10px] text-[13px] rounded-full focus:outline-none font-niradei bg-[#343433] text-white cursor-pointer"
            >
              Reset
            </button>
        </div>
