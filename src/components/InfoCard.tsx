        ...
        <div className="modal-box p-0">
          ...
          <div className="flex flex-col  items-center ">
              <div className="h-[200px] w-full bg-red-400 mb-20 relative">
                <Image src={heroImgUrl}  className="object-cover w-full h-full " width={10000} height={10000} alt="" />
                ...
                <Image className="object-cover " src={logoImgUrl}  alt="" width={1000} height={1000} />
                  </div>                
              </div>
              <p className="font-akbalthom-moul-4 text-center text-2xl ">{title}</p>
              <p className="px-5 font-battambong font-light mt-5 text-sm text-center"> {t.contactTeam} <br />
              {t.contactSocialmedia}</p>
              <div className="flex flex-row space-x-3 mt-5">
                {
                    icons?.map((social: any) => (
                         <Link key={social.logo} href={social.url}>
                           <Image className="w-10"  src={`/icons/${social.logo}`} alt=""
                                      width={100}
                                      height={100}/>
              ...
              <div className="flex font-battambong space-x-2 mt-5  p-4 w-full justify-center bg-orange-100">
                <Image className="w-5" src={"/icons/call.svg"} alt="" width={100} height={100} />

                <h1 className="text-orange-500">{t.contactPhone} / Hotline: <Link href={`tel:+855${info?.phone_num}`}>{info?.phone_num}</Link> </h1>
              </div>
          </div>
        </div>
      </dialog>
