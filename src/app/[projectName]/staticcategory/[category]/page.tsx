...
function StaticCategory() {
  ...
  const [menuData, setMenuData] = useState<MenuType>([]); // Separate state for Menubar
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await axios.get(
          `https://${projectName}.tsdsolution.net/api/DriverController/getMainProducts?tag=${category}`
        );
        const dataJson: MenuType = response.data;
        console.log("Data fetched for category:", category, dataJson);

        if (dataJson.length === 0 || !dataJson.some((cat) => cat.items.length > 0)) {
          setNotFound(true);
        } else {
          setData(dataJson);
  ...
  const processFiltering = useCallback(() => {
    ...
    if (isInstockOnly) {
      filteredItems = filteredItems.map((cat) => ({
        category: cat.category,
        items: cat.items.filter(
          (item) => !isNaN(parseFloat(item.quantity)) && parseFloat(item.quantity) > 0
        ),
      })).filter((cat) => cat.items.length > 0);
    }
    ...
    if (filters.pieces.length > 0) {
      filteredItems = filteredItems.map((cat) => ({
        ...
        items: cat.items.filter((item) => {
          ...
          return filters.pieces.some((piecesFilter) => {
            switch (piecesFilter) {
              case "0 - 99": return pieces >= 0 && pieces <= 99;
              case "100 - 499": return pieces >= 100 && pieces <= 499;
              case "500 - 999": return pieces >= 500 && pieces <= 999;
              case "1000+": return pieces >= 1000;
    ...
    if (filters.age.length > 0) {
      filteredItems = filteredItems.map((cat) => ({
        ...
        items: cat.items.filter((item) => {
          ...
          return filters.age.some((ageFilter) => {
            switch (ageFilter) {
              case "0 - 6": return age >= 0 && age <= 6;
              case "6 - 9": return age >= 6 && age <= 9;
              case "10 - 17": return age >= 10 && age <= 17;
    ...
    filteredItems = filteredItems.map((cat) => ({
      ...
      items: [...cat.items].sort((a, b) => {
        ...
        const piecesA = a.pieces, piecesB = b.pieces;
        ...
        switch (sortData) {
          ...
          case "Piece count: Low to High": return piecesA - piecesB;
          case "Piece count: High to Low": return piecesB - piecesA;
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const offsets = ref.current.map((el) => (el ? el.getBoundingClientRect().top : Infinity));
